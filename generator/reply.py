...
- 답글은 댓글 내용 자체에만 반응해서 끝내. "자주 소통해요"·"놀러와줘요"·"좋은 하루 보내요" 같은
  덧붙임 끝인사는 절대 쓰지 마 — 모든 답글에 똑같이 붙으면 봇처럼 보인다(끝인사는 시스템이 가끔 붙임).
  '맞팔해요', '스하리 환영', '팔로우' 같은 인위적인 단어도 절대 금지.

━━ 예시 (이 느낌으로) ━━
  댓글 "나도 카레 좋아해 ㅋㅋ" → "ㅎㅎ 카레 최고야 나도 요즘 자주 해먹어!"
  댓글 "이거 어디서 사?" → "프로필 링크에 깔끔하게 표로 정리해뒀어~ 꼭 확인해보고 돈 아껴!!"
  댓글 "진짜 편하겠다" → "맞아 이거 쓰고 나서 아침이 진짜 편해졌어 ㅎㅎ"
  댓글 "스하리 와요" / "반가워요" → "헐 너무 반가워요!! 어떻게 오셨어요 ㅎㅎ"
...
# 끝인사는 프롬프트가 아니라 코드가 관리(2026-07-17 사용자 피드백: LLM이 예시를 따라해
# 모든 답글에 "우리 자주 소통해요 ㅎㅎ"가 붙어 봇티 남). 확률 15%, 문구 로테이션 + LLM 잔재 제거.
_CLOSER_STRIP_RE = re.compile(
    r"[\s,~!]*(?:우리\s*)?(?:자주\s*소통해요|자주\s*놀러와\s*줘?요|앞으로\s*자주\s*소통해요"
    r"|오늘도\s*좋은\s*하루\s*보내요)[\s~!ㅎㅋ🥰💛❤️♥]*$"
)
_CLOSERS = [
    "오늘 하루 잘 마무리해요 ㅎㅎ", "댓글 남겨줘서 고마워요 ㅎㅎ", "또 봐요 우리 ㅎㅎ",
    "덕분에 기분 좋아졌어요 ㅎㅎ", "다음 글에서 또 만나요 🙂", "좋은 저녁 보내요~",
]
_CLOSER_PROB = 0.15


def _apply_closer(reply: str) -> str:
    """LLM이 붙인 상투 끝인사는 떼고, 낮은 확률로만 다양한 끝인사를 붙인다."""
    import random
    base = _CLOSER_STRIP_RE.sub("", reply).strip()
    if not base:
        base = reply.strip()
    if random.random() < _CLOSER_PROB:
        return f"{base} {random.choice(_CLOSERS)}"
    return base
