...
## ★2026-07-17 겹침·꼬임 4종 수정 (사용자 피드백, 71ea7e3 + osmu 4a64759)
- **미발행 카탈로그 노출 근절**: osmu `assign_and_deploy_landing`이 게시 전에 mark_posted까지
  해서 대기열 상품(063~070)이 랜딩에 노출됐었음 → 코드 할당만 하고, **posted+랜딩 재생성은
  게시 성공 후 `finalize_landing()`**. 063~070 posted=false 복구+페이지 재생성 완료.
- **같은 날 사진+영상 겹침(레이스)**: 게이트는 양쪽 다 있었으나 판정→발행 사이 몇 분의
  TOCTOU(07-16 저녁 062영상+071사진 실사고) → auto/evening **발행 직전
  `refresh_shared_feed()`(git pull)+`coupang_posted_today` 재판정** 추가.
- **영상 첫 댓글 = 쿠팡 직링크**(사진 상품글과 동일). 랜딩 링크는 URL 무효 시 폴백만.
  `/np/search` 검색URL은 쿼리 있어도 발행 차단(062가 검색링크로 나간 원인).
  ⚠️영상 상품 URL은 대부분 순정 coupang.com(파트너스 태그 없음) — 수수료 잡히려면
  파트너스 OpenAPI 딥링크 필요(키 미발급, 사용자 액션).
- **캡션**: '상품명 언급 금지'가 범용 미끼문구·비문을 양산 → **제품군 단어(_product_noun)
  1회 필수**(프롬프트+게이트+폴백), 번역투 금지 명문화.
- **대댓 끝인사**: 프롬프트에서 제거(LLM이 예시 따라해 전부 "우리 자주 소통해요") →
  코드에서 15% 확률·6종 로테이션(`reply._apply_closer`).
- 남은 확인: 대기열의 [063] vs [066] 소파베드 유사 상품 중복 — 대시보드에서 하나 반려 권장.
