...
async def run():
    ...
    # 이미지 구성(2026-07-13 개편): 실사진 우선, AI는 폴백.
    #   쿠팡 상세 실사진(detail_imgs, 수집 성공 시 3~4장)이 있으면 그대로 carousel —
    #   AI 변형컷의 실물 미세 왜곡·연출 이미지 리스크 회피.
    #   상세 수집이 대표 1장 폴백에 그친 경우에만 AI 보조 컷으로 구색 보강.
    ...
    refresh_shared_feed(label="auto")
    if coupang_posted_today(label="auto(직전 재판정)"):
        return
